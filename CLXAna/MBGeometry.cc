...
void MBGeometry::SetupCluster(double user_theta, double user_phi,
                              double user_alpha, double user_r, double user_z) {
...
#define USE_EMPIRICAL 0
  ...
  Double_t R =
      DIST_CORE_CORNER *
      0.5; // average distance from centre of capsule to centre of segment

  det_offset[0].SetXYZ(0, r + 9, -DIST_CORE_CORNER);
  det_offset[1].SetXYZ(-DIST_CORE_CORNER * ROOTTHREEOVER2, r + 9,
                       DIST_CORE_CORNER / 2);
  det_offset[2].SetXYZ(DIST_CORE_CORNER * ROOTTHREEOVER2, r + 9,
                       DIST_CORE_CORNER / 2);
  ...
  for (UInt_t i = 0; i < 3; i++)
    det_offset[i] += mbzoffset;
  for (UInt_t i = 0; i < 18; i++)
    seg_offset[i] += mbzoffset;
