...
void hists::FillTree( float GEn, float GTh, float GPh, int GCluid, int GCid, int GSid,
		      vector <float> GCor_GEn, vector <float> GCor_GTh,
		      vector <float> GCor_GPh, vector <int> GCor_GCluID, vector <int> GCor_GCid, vector <int> GCor_GSid, vector <float> GCor_Gtd,
		      vector <int> Laser, vector <float> PEn, vector<int> Pnf, vector<int> Pnb, vector<int> Psec, vector<int> Pquad, vector<float> Ptd){
  ...
  if (np_passed==1){
    // for(int i = 0; i < np_passed; i++){
    //   cout<<i<<" Ep: "<<PEn_passed[i]/1000<<" MeV, td (ticks): "<<Ptd_passed[i]<<", PID: "<<Ppid_passed[i]<<", quad: "<<Pquad_passed[i]<<", Eg: "<<GEn<<" keV, Ng: "<<GCor_GEn.size()+1<<endl;
    // }
    np = 1;
    //    b2p = 0;
    tdpp = 0.;
  ...
  else if (np_passed==2){ // 2-particle case, check quadrant correlation (diff = 2) and in time (ppwin, in ns) ; separate if 1n
    //   cout<<i<<" Ep: "<<PEn_passed[i]/1000<<" MeV, td (ticks): "<<Ptd_passed[i]<<", PID: "<<Ppid_passed[i]<<", quad: "<<Pquad_passed[i]<<", Eg: "<<GEn<<" keV, Ng: "<<GCor_GEn.size()+1<<endl;
    ...
    int cut2 = dc.Cut_2p(PEn_passed[0],Pnf_passed[0],Pquad_passed[0],Psec_passed[0],
			 PEn_passed[1],Pnf_passed[1],Pquad_passed[1],Psec_passed[1]); // returns 0 for target-beam passed, 1 for beam/target passed, -1 for small 2p angles (ring > 10 (innermost = 16) for both)
	...
	int cut2 = dc.Cut_2p(PEn_passed[j],Pnf_passed[j],Pquad_passed[j],Psec_passed[j],
			     PEn_passed[k],Pnf_passed[k],Pquad_passed[k],Psec_passed[k]); // returns 0 for target-beam passed, 1 for beam/target passed, -1 for small 2p angles (ring > 10 (innermost = 16) for both)
    ...
    //    for (int i = 0; i < v2p.size(); i++){
      //      cout<<"2p: |dT1-dT2| = "<<TMath::Abs(Ptd_passed[v2p[i].first]-Ptd_passed[v2p[i].second])<<" Q = "<<Pquad_passed[v2p[i].first]<<" vs "<<Pquad_passed[v2p[i].second]<<" cut = "<<Ppid_passed[v2p[i].first]<<"/"<<Ppid_passed[v2p[i].second]<<" Ep: "<<PEn_passed[v2p[i].first]/1000<<" vs "<<PEn_passed[v2p[i].second]/1000<<endl;
    // }
    // for (int i = 0; i < v1p.size(); i++){
      //      cout<<"1p: dT = "<<Ptd_passed[v1p[i]]<<" Q = "<<Pquad_passed[v1p[i]]<<" cut = "<<Ppid_passed[v1p[i]]<<endl;
    //    }
    // fill 1p events
...
//   float time_diff = TMath::Abs(td[Pptr[0]]-td[Pptr[1]]); // 2p time difference in ns
//   int quad_diff = TMath::Abs(Pquad[Pptr[0]]-Pquad[Pptr[1]]); // quadrant number difference
//   int cut2 = dc.Cut_2p(PEn[Pptr[0]],Pnf[Pptr[0]],Pquad[Pptr[0]],Psec[Pptr[0]],
// 		       PEn[Pptr[1]],Pnf[Pptr[1]],Pquad[Pptr[1]],Psec[Pptr[1]]); // returns 0 for target-beam passed, 1 for beam/target passed, -1 for small 2p angles (ring > 10 (innermost = 16) for both)
//   bool cutg_0 = dc.CutG_en2hit(PEn[Pptr[1]]/1000.,PEn[Pptr[0]]/1000.); // set to true always (no energy correlation cut)
...
//   // gamma spectra
//   B_dcB->Add(B_1hdcB,T_2hdcB); // 1- and 2-hit beam particle detected, DC'ed to beam
//   B_dcT->Add(B_1hdcT,T_2hdcT); // 1- and 2-hit beam particle detected, DC'ed to target
//   T_dcB->Add(T_1hdcB,T_2hdcB); // 1- and 2-hit target particle detected, DC'ed to beam
//   T_dcT->Add(T_1hdcT,T_2hdcT); // 1- and 2-hit target particle detected, DC'ed to target
//   gam_dcB->Add(B_dcB,T_1hdcB); // beam/target detected, DC'ed to beam
//   gam_dcT->Add(T_dcT,B_1hdcT); // beam/target detected, DC'ed to target
