...
ClassImp(DgfSubEvent) ClassImp(AdcSubEvent) ClassImp(PatternUnitSubEvent)
...
ostream &operator<<(ostream &os, const DgfSubEvent &obj) {
  ...
  for (unsigned short i = 0; i < NumberOfChannels; i++) {
    cout << "Channel " << i << ": Energy = " << obj.Energy[i]
         << ", FastTriggerTime = ";
    cout << obj.FastTriggerTime[i]
...
PatternUnitSubEvent::~PatternUnitSubEvent() { Values.clear(); }
