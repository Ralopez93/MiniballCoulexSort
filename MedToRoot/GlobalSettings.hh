...
class GlobalSettings : public TObject {
 public:
  GlobalSettings(){};
  GlobalSettings(unsigned int, char*[]);
  ~GlobalSettings();

  friend ostream& operator<<(ostream&, const GlobalSettings&);

  inline unsigned short FirstMiniballDgf() { return fFirstMiniballDgf; };

  inline unsigned short LastMiniballDgf() { return fLastMiniballDgf; };

  inline unsigned short BeamdumpDgf() { return fBeamdumpDgf; };

  inline unsigned short FirstAdc() { return fFirstAdc; };

  inline unsigned short LastAdc() { return fLastAdc; };

  inline unsigned short BraggChamber() { return fBraggChamber; };

  inline unsigned short Scaler() { return fScaler; };

  inline unsigned short PatternUnit() { return fPatternUnit; };

  inline unsigned short FirstDgfScaler() { return fFirstDgfScaler; };

  inline unsigned short LastDgfScaler() { return fLastDgfScaler; };

  inline unsigned short NumberOfDgfModules() { return fNumberOfDgfModules; };

  inline unsigned short NumberOfAdcModules() { return fNumberOfAdcModules; };

  inline unsigned short NumberOfPatternUnits() {
    return fNumberOfPatternUnits;
  };

  inline unsigned short NumberOfDgfChannels() { return fNumberOfDgfChannels; };
  ...
  inline unsigned short TimestampChannel() { return fTimestampChannel; };
  ...
  inline unsigned short EbisChannel() { return fEbisChannel; };

  inline unsigned short T1Channel() { return fT1Channel; };

  inline unsigned short SuperCycleChannel() { return fSuperCycleChannel; };
  ...
  inline long long FirstEvent() { return fFirstEvent; };

  inline long long LastEvent() { return fLastEvent; };

  inline int VerboseLevel() { return fVerboseLevel; };

  inline const char* MedFile() { return fMedFile.c_str(); };

  inline const char* OnBeamFile() { return fOnBeamFile.c_str(); };

  inline const char* OnBeamBackgroundFile() {
    return fOnBeamBackgroundFile.c_str();
  };

  inline const char* OffBeamFile() { return fOffBeamFile.c_str(); };

  inline const char* ScalerFile() { return fScalerFile.c_str(); };
  ...
  unsigned short RealTimeIndex() { return fRealTimeIndex; };

  unsigned short RunTimeIndex() { return fRunTimeIndex; };

  unsigned short GSLTTimeIndex() { return fGSLTTimeIndex; };

  unsigned short NumberOfEventsIndex() { return fNumberOfEventsIndex; };

  unsigned short ChannelIndicesOffset(unsigned short channel) {
    if (channel >= fNumberOfDgfChannels) {
      cerr << "Error, channel " << channel << " out of range (0 - "
           << fNumberOfDgfChannels << ")" << endl;
      return 0xffff;
    }
    return fChannelIndicesOffset[channel];
  };

  unsigned short LiveTimeIndex() { return fLiveTimeIndex; };

  unsigned short FastPeakIndex() { return fFastPeakIndex; };

  unsigned short BraggTraceLength() { return fBraggTraceLength; };

  bool SourceRun() { return fSourceRun; };

  bool IncludeScaler() { return fIncludeScaler; };

  bool BeamDumpOn() { return fBeamDumpOn; };

  bool MesytecAdc() { return fMesytecAdc; };

  bool SPEDEChamb() { return fSPEDEChamb; };

  unsigned short NofCaenAdc() { return fNofCaenAdc; };

  unsigned short EbisWindowLowerEdge() { return fEbisWindowLowerEdge; };

  unsigned short EbisWindowUpperEdge() { return fEbisWindowUpperEdge; };

  long long DgfInitDelay() { return fDgfInitDelay; };
