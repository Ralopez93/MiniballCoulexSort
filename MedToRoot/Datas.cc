...
ClassImp(AdcData) ClassImp(DgfData) ClassImp(AdcSubEvent)
...
AdcData::AdcData(unsigned short moduleNumber, long long time,
                 AdcSubEvent* adcSubEvent, bool LaserOn, bool FieldUp,
                 bool FieldDown)
    : fModuleNumber(moduleNumber),
      fTime(time),
      fLaserOn(LaserOn),
      fFieldUp(FieldUp),
      fFieldDown(FieldDown) {
  //  cout<<__PRETTY_FUNCTION__<<endl;
...
DgfData::DgfData(unsigned short moduleNumber, unsigned short channel,
                 DgfSubEvent* dgfSubEvent, unsigned short multiplicity)
    : fModuleNumber(moduleNumber),
      fChannel(channel),
