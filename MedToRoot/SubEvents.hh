...
class DgfSubEvent : public TObject {
 public:
  ...
  inline void SetEventTime(unsigned short RunTime, unsigned short EventTimeHigh,
                           unsigned short EventTimeLow) {
    EventTime =
        EventTimeLow + 65536ll * EventTimeHigh + 65536ll * 65536ll * RunTime;
  };

  inline void SetFastTriggerTime(unsigned short channelNumber,
                                 unsigned short fastTriggerTime) {
    if (channelNumber < NumberOfChannels)
      FastTriggerTime[channelNumber] = fastTriggerTime;
    else {
      cerr << __PRETTY_FUNCTION__ << ": wrong channel number " << channelNumber
           << " (0-" << NumberOfChannels - 1 << ")" << endl;
    }
  };
  ...
  inline void SetLongFastTriggerTime(unsigned short channelNumber,
                                     unsigned short RunTime,
                                     unsigned short EventTimeHigh,
                                     unsigned short EventTimeLow) {
    if (channelNumber < NumberOfChannels) {
      // check for overflow
      if (FastTriggerTime[channelNumber] > EventTimeLow)
        LongFastTriggerTime[channelNumber] = FastTriggerTime[channelNumber] +
                                             65536ll * EventTimeHigh +
                                             65536ll * 65536ll * RunTime;
      else
        LongFastTriggerTime[channelNumber] = FastTriggerTime[channelNumber] +
                                             65536ll * EventTimeHigh + 65536ll +
                                             65536ll * 65536ll * RunTime;
    } else {
      cerr << __PRETTY_FUNCTION__ << ": wrong channel number " << channelNumber
           << " (0-" << NumberOfChannels - 1 << ")" << endl;
  ...
  inline unsigned short GetNumberOfChannels() { return NumberOfChannels; };
  ...
  inline void GetUserValues(unsigned short channelNumber,
                            unsigned short* userValues) {
    if (channelNumber < NumberOfChannels) {
      for (unsigned short i = 0; i < NumberOfUserValues; i++) {
 ...
 public:
  ...
  inline size_t Size() {
    if (fAdcChannel.size() == fAdcValue.size())
      ...
      cerr << "Error, AdcChannel and AdcValue have different sizes ("
           << fAdcChannel.size() << " != " << fAdcValue.size() << ")" << endl;
      return 0;
    }
  };
  ...
  inline void Timestamp(long long timeStamp) { fTimestamp = timeStamp; };

  // get functions
  inline vector<unsigned short> AdcChannel() { return fAdcChannel; }

  inline vector<unsigned short> AdcValue() { return fAdcValue; }
  ...
  inline unsigned short GetLastAdcChannel() { return fAdcChannel.back(); };

  inline unsigned short GetLastAdcValue() { return fAdcValue.back(); };

  inline void Get(unsigned short Index, unsigned short& adcChannel,
    ...
    if (Index < Size()) {
      ...
      cerr << __PRETTY_FUNCTION__ << ": Index " << Index << " out of bounds (0-"
           << Size() << ")" << endl;
      adcChannel = 0xffff;
      adcValue = 0;
    }
  ...
  inline long long Timestamp() { return fTimestamp; };
...
class PatternUnitSubEvent : public TObject {
 public:
  ...
  // Reset
  inline void ClearEvt() { Values.clear(); };

  inline void Add(unsigned int Value) { Values.push_back(Value); };

  inline unsigned int GetValue(unsigned short Index) {
    return Values.at(Index);
  };

  inline unsigned int GetLastValue() { return Values.back(); };
...
class ScalerSubEvent : public TObject {
 public:
  ...
  inline size_t Size() {
    if (Channels.size() == Values.size())
      ...
      cerr << __PRETTY_FUNCTION__
           << ": Error, size of channels and values is not the same ("
           << Channels.size() << ", " << Values.size() << ")" << endl;
      exit(1);
    }
  };
  ...
  inline unsigned short GetLastChannel() { return Channels.back(); };
...
class DgfScalerSubEvent : public TObject {
 ...
 public:
  ...
  // set functions
  void ModuleNumber(unsigned short DgfID) { fModuleNumber = DgfID; };

  void ClusterID(unsigned short ClusterID) { fClusterID = ClusterID; };

  void RealTime(long long time) { fRealTime = time; };

  void RunTime(long long time) { fRunTime = time; };

  void GSLTTime(long long time) { fGSLTTime = time; };
  ...
  void LiveTime(unsigned short i, long long time) {
    if (i < fNumberOfDgfChannels) {
      ...
      cerr << __PRETTY_FUNCTION__ << ": Error, trying to write to index " << i
           << " of " << fNumberOfDgfChannels << endl;
      return;
    }
  };
  ...
  // get functions
  unsigned short ModuleNumber() { return fModuleNumber; };

  unsigned short ClusterID() { return fClusterID; };

  long long RealTime() { return fRealTime; };

  long long RunTime() { return fRunTime; };

  long long GSLTTime() { return fGSLTTime; };

  unsigned short NumberOfEvents() { return fNumberOfEvents; };

  long long LiveTime(unsigned short i) {
    if (i < fNumberOfDgfChannels) {
      return fLiveTime[i];
    } else {
      cerr << __PRETTY_FUNCTION__ << ": Error, trying to read from index " << i
           << " of " << fNumberOfDgfChannels << endl;
      return 0;
    }
  };
...
class BraggChamberSubEvent : public TObject {
 public:
  ...
  // Reset
  inline void ClearEvt() {
    for (unsigned short i = 0; i < NumberOfBraggChannels; i++) fData[i].clear();
  };

  // set functions
  void DataSize(unsigned short Size) { fDataSize = Size; };

  void Trigger(unsigned short i, bool Trigger) {
    if (i < NumberOfBraggChannels) {
      fTrigger[i] = Trigger;
    } else {
      cerr << __PRETTY_FUNCTION__ << ": channel can only be 0 or 1, not " << i
           << endl;
    }
  };
  ...
  unsigned short DataSize() { return fDataSize; };
