...
#define MESYTEC_MADC_ADC_RESOLUTION \
  0x7000  // 0=2k,1=4k,2=4k hires,3=8k,4=8k hires
...
class UnpackedEvent : public TObject {
 public:
  ...
  void SetDgfModule(size_t Index, DgfModule NewDgfModule) {
    if (Index <= Settings->NumberOfDgfModules()) {
      ...
      cerr << __PRETTY_FUNCTION__ << ": Error, index " << Index
           << " out of bounds (0 - " << Settings->NumberOfDgfModules() << ")"
           << endl;
    }
  };
  ...
  void SetAdcModule(size_t Index, AdcModule NewAdcModule) {
    if (Index <= Settings->NumberOfAdcModules()) {
      ...
      cerr << __PRETTY_FUNCTION__ << ": Error, index " << Index
           << " out of bounds (0 - " << Settings->NumberOfAdcModules() << ")"
           << endl;
    }
  };
  ...
  void SetPatternUnit(size_t Index, PatternUnit NewPatternUnit) {
    if (Index <= Settings->NumberOfPatternUnits()) {
      ...
      cerr << __PRETTY_FUNCTION__ << ": Error, index " << Index
           << " out of bounds (0 - " << Settings->NumberOfPatternUnits() << ")"
           << endl;
    }
  };
  ...
  void ClearEvt() {
    if (Settings->VerboseLevel() > 3) {
      cout << "clearing " << Settings->NumberOfDgfModules() << " dgfs, "
           << Settings->NumberOfAdcModules() << " adcs and "
           << Settings->NumberOfPatternUnits() << " pattern units" << endl;
    }
  ...
  // accessing modules
  long long GetEventNumber() { return EventNumber; }

  int GetNumberOfDgfModules() { return Settings->NumberOfDgfModules(); };

  int GetNumberOfAdcModules() { return Settings->NumberOfAdcModules(); };

  int GetNumberOfPatternUnits() { return Settings->NumberOfPatternUnits(); };
  ...
  DgfModule* GetTimestampModule(size_t Index) {
    if (Index <= Settings->NumberOfTimestampModules()) {
      ...
      cerr << __PRETTY_FUNCTION__ << ": Error, index " << Index
           << " out of bounds (0 - " << Settings->NumberOfTimestampModules()
           << ")" << endl;
      return NULL;
    }
  };
  ...
  bool ScalerData() { return fScalerData; };

  SISScaler* GetScaler() { return &Scaler; };

  DgfScaler* GetDgfScaler() { return &fDgfScaler; };

  BraggChamber* GetBraggChamber() { return &fBraggChamber; };
 ...
 protected:
  ...
  // variable to store buffer time of the first buffer (to correct mesytec adc
  // time)
