...
ClassImp(GlobalSettings) ClassImp(BuiltEvent) ClassImp(EventBuffer)
...
EventBuffer::EventBuffer(GlobalSettings* settings) : Settings(settings) {
  cout << __PRETTY_FUNCTION__ << endl;
  Sorted = false;
  SubEventNumber = NULL;
}
...
size_t EventBuffer::AddParticle(unsigned short moduleNumber,
                                AdcSubEvent* adcSubEvent, long long time,
                                bool LaserOn, bool FieldUp, bool FieldDown) {
  ...
  for (size_t event = 0; event < BuiltEvents.size(); event++) {
    if (BuiltEvents[event].Coincident(Settings, time)) {
      BuiltEvents[event].AddParticle(moduleNumber, adcSubEvent, time, LaserOn,
                                     FieldUp, FieldDown);
      return BuiltEvents.size();
    }
  }

  BuiltEvents.push_back(
      BuiltEvent(moduleNumber, adcSubEvent, time, LaserOn, FieldUp, FieldDown));

  return BuiltEvents.size();
}

size_t EventBuffer::AddParticle(unsigned short moduleNumber,
                                AdcSubEvent* adcSubEvent, bool LaserOn,
                                bool FieldUp, bool FieldDown) {
  // check whether this particle is coincident with one of the others
  for (size_t event = 0; event < BuiltEvents.size(); event++) {
    if (BuiltEvents[event].Coincident(Settings, adcSubEvent->Timestamp())) {
      BuiltEvents[event].AddParticle(moduleNumber, adcSubEvent,
                                     adcSubEvent->Timestamp(), LaserOn, FieldUp,
                                     FieldDown);
      return BuiltEvents.size();
    }
  }

  BuiltEvents.push_back(BuiltEvent(moduleNumber, adcSubEvent,
                                   adcSubEvent->Timestamp(), LaserOn, FieldUp,
                                   FieldDown));
...
size_t EventBuffer::AddGamma(unsigned short moduleNumber,
                             DgfSubEvent* dgfSubEvent) {
  unsigned short multiplicity;

  // loop over all channels
  for (size_t channel = 0; channel < dgfSubEvent->GetNumberOfChannels();
       channel++) {
    // check whether channel has a hit and whether energy is greated than zero
    if ((dgfSubEvent->GetHitPattern() & 1 << channel) != 0 &&
        dgfSubEvent->GetEnergy(channel) > 0) {
      // to count how often this gamma was filled into the event
      multiplicity = 1;
      for (size_t event = 0; event < BuiltEvents.size(); event++) {
        if (BuiltEvents[event].Coincident(
                Settings, dgfSubEvent->GetLongFastTriggerTime(channel))) {
          BuiltEvents[event].AddGamma(moduleNumber, channel, dgfSubEvent,
                                      multiplicity);
          multiplicity++;
        }
      }

      // this means the loop ended without finding a coincident particle => add
      // a new event
      if (multiplicity == 1) {
...
void EventBuffer::AddTiming(long long EbisTime, long long T1Time,
...
void EventBuffer::Sort() {
  ...
  // simply use the subevent number for sorting, the actual sorting is done
  // later!
      ...
      if (BuiltEvents[i].GetTime() < BuiltEvents[SubEventNumber[j]].GetTime()) {
        ...
        for (k = i; k > j; k--) {
          ...
          if (Settings->VerboseLevel() > 4) {
            cout << "i = " << i << ", j = " << j << ", k = " << k
                 << ", SubEventNumber[";
            cout << k << "] = " << SubEventNumber[k] << ", SubEventNumber["
                 << k - 1;
  ...
  if (Settings->VerboseLevel() > 3) {
    ...
    cout << " got following order (first line = index, second line = time):"
         << endl;
...
BuiltEvent* EventBuffer::GetSortedEvent(size_t Index) {
  if (Sorted) {
    if (BuiltEvents.size() < Index) {
      cerr << "Index " << Index << " is out of range 0 - " << BuiltEvents.size()
           << endl;
      return NULL;
    }
    ...
    cerr << "Sorry you first have to sort the buffer to get an sorted event!"
         << endl;
    return NULL;
  }
}
