...
int main(int argc, char *argv[]) {
  ...
  if (mbs == NULL)
    printf("couldn't open file %s -> exit!!!\n", Settings->MedFile());
  ...
  while (mbs != NULL) {
    ...
    if (nofProcessedEvents % 10000 == 0) {
      cout << "Processing event number " << setw(7) << nofProcessedEvents
           << "\r";
      cout.flush();
    }
    ...
    // check #of processed events
    if (nofProcessedEvents == Settings->LastEvent()) break;

    // if stop trigger was found or an illegal trigger => stop
    if (status == 15 || status == 2) break;
