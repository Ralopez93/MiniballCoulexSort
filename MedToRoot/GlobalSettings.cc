...
ClassImp(GlobalSettings)
  ...
  interface->Add("-fe", "first event to be read out", &fFirstEvent);
  ...
  interface->Add("-ic", "include scaler data into on/offbeam trees",
                 &fIncludeScaler);
  ...
  if (fMedFile.empty()) {
    cerr << "Error, name of the MED-file (" << fMedFile << ") not provided!"
         << endl;
    exit(1);

  }

  else if (fSettingsFile.empty()) {
    cerr << "Error, name of the settings-file (" << fSettingsFile
         << ") not provided!" << endl;
    exit(1);
  }
  ...
  fOnBeamBackgroundFile.replace(LastDot, fMedFile.length() - LastDot,
                                "_OnBeamBackground.root");
  ...
  if (fVerboseLevel > 0) {
    ...
    for (i = 0; i < fNumberOfTimestampModules; i++)
      cout << i + 1 << ". module: module number = " << fTimestampModule[i]
           << ", offset = " << fTimestampModuleOffset[i] << endl;
    cout << "Timestamp Channel = " << fTimestampChannel << endl
         << "# of Adcs per Timestamp = " << fNumberOfAdcsPerTimestampModule
         << endl
         << "Ebis, T1 and SuperCycle module = " << fEbisT1AndSuperCycleModule
         << endl
  ...
  if (!Verify()) {
    cerr << "Sorry, the settings file " << fSettingsFile
         << " is not of the correct format" << endl;
    exit(1);
  }
}
...
GlobalSettings::~GlobalSettings() {
  ...
  //  if(fVerboseLevel > 2)
  //	{
  //	  cout<<"called ~GlobalSettings with fTimestampModule =
  //"<<fTimestampModule<<" and fTimestampModuleOffset =
  //"<<fTimestampModuleOffset<<endl;
  //	}
  //
...
void GlobalSettings::ReadUnpackerSettings() {
  if (fSettings == NULL) {
    cerr << __PRETTY_FUNCTION__
         << ": you need to create a new TEnv first, fSettings in NULL!" << endl;
    exit(1);
  }
  ...
  fHighestCoincidence = (long long)(fReferencePointOfCoincidenceWindow +
                                    fCoincidenceWindowWidth / 2);
  ...
  fNumberOfAdcsPerTimestampModule =
      fSettings->GetValue("Timestamp.NumberOfAdcs", 3);
  ...
  for (unsigned short i = 0; i < fNumberOfDgfChannels; i++)
    fChannelIndicesOffset[i] =
        fSettings->GetValue(Form("Dgf.Indices.Offset.%d", i), -1);
...
void GlobalSettings::ReadSettings() {
  ...
  fNumberOfTimestampModules =
      fSettings->GetValue("Timestamp.NumberOfModules", 2);
  ...
  for (unsigned short i = 0; i < fNumberOfTimestampModules; i++) {
    fTimestampModule[i] =
        fSettings->GetValue(Form("Timestamp.%d.ModuleNumber", i), 49);
    fTimestampModuleOffset[i] =
        fSettings->GetValue(Form("Timestamp.%d.Offset", i), 0.);
  }
...
ostream& operator<<(ostream& os, const GlobalSettings& obj) {
  ...
  for (unsigned short i = 0; i < obj.fNumberOfTimestampModules; i++)
    os << i + 1 << ". module: module number = " << obj.fTimestampModule[i]
       << ", offset = " << obj.fTimestampModuleOffset[i] << endl;

  os << "Timestamp Channel = " << obj.fTimestampChannel << endl
     << "# of Adcs per Timestamp = " << obj.fNumberOfAdcsPerTimestampModule
     << endl
     << "Ebis, T1 and SuperCycle module = " << obj.fEbisT1AndSuperCycleModule
     << endl
...
bool GlobalSettings::Verify() {
  ...
  for (unsigned short i = 0; i < fNumberOfTimestampModules; i++) {
    if (fTimestampModule[i] > fNumberOfDgfModules) {
      cerr << __PRETTY_FUNCTION__
           << ": Error, inconsistent settings: TimestampModule ";
      cerr << i << " = " << fTimestampModule[i]
           << ", but total number of dgf modules is only ";
      cerr << fNumberOfDgfModules << endl;
      return false;
    }
  }
