...
ClassImp(GlobalSettings) ClassImp(DgfModule) ClassImp(AdcModule)
    ClassImp(PatternUnit) ClassImp(UnpackedEvent) ClassImp(DgfSubEvent)
        ClassImp(AdcSubEvent) ClassImp(PatternUnitSubEvent)
  ...
  switch (mbs_get_event_trigger(mbs)) {
    ...
    case 1:  // (=kMrbTriggerReadout in DgfCommonIndices.h)
      ...
      if (Settings->VerboseLevel() > 3) {
        cout << "starting to extract subevents of event " << EventNumber
             << endl;
      }

      if (ExtractSubevents(mbs)) {  // extract subevents

        // We should not correct the Mesytec timestamps, but the DGF ones
        // instead
    ...
    default:
      ...
      cout << "Illegal trigger number - #" << mbs_get_event_trigger(mbs)
           << endl;
      return 2;
  }
}
...
bool UnpackedEvent::ExtractSubevents(const MBSDataIO* mbs) {
  ...
  while (true) {
    ...
    if (sevtType == MBS_STYPE_ABORT) {  // error
      ...
      cout << "sevtType == MBS_STYPE_ABORT after call of 'mbs_next_sheader()'"
           << endl;
      return false;
    }
    ...
    if (sevtType == MBS_STYPE_ERROR) {  // error
      ...
      cout << "sevtType == MBS_STYPE_ERROR after call of 'mbs_next_sdata()"
           << endl;
      return false;
    }
    ...
    // Dgf data
    if ((Settings->FirstMiniballDgf() <= mbs->sevt_id &&
         mbs->sevt_id <= Settings->LastMiniballDgf()) ||
      ...
      if (Settings->VerboseLevel() > 2) {
        cout << "DGF mbs->sevt_id " << mbs->sevt_id << " mbs->sevt_wc "
             << mbs->sevt_wc << endl;
      }
    ...
    // ADC & TDC data
    else if (Settings->FirstAdc() <= mbs->sevt_id &&
             mbs->sevt_id <= Settings->LastAdc() - Settings->NofCaenAdc()) {
      if (!Settings->MesytecAdc()) {
        ...
        if (Settings->VerboseLevel() > 2) {
          cout << "MADC mbs->sevt_id " << mbs->sevt_id << " mbs->sevt_wc "
               << mbs->sevt_wc << endl;
        }
    ...
    else if (Settings->LastAdc() - Settings->NofCaenAdc() < mbs->sevt_id &&
             mbs->sevt_id <= Settings->LastAdc()) {
      if (Settings->VerboseLevel() > 2) {
    ...
    else if (Settings->FirstDgfScaler() <= mbs->sevt_id &&
             mbs->sevt_id <= Settings->LastDgfScaler()) {
    ...
    // other
      ...
      cout << ", not " << Settings->FirstMiniballDgf() << " - "
           << Settings->LastMiniballDgf();
      ...
      cout << " and not " << Settings->FirstDgfScaler() << " - "
           << Settings->LastDgfScaler() << endl;
    }
  }
}

// subevent id 1-9, type [10,23]: raw dgf data, format 0x101
bool UnpackedEvent::DecodeDgf(int SubEventID, int SubEventWordCount,
                              char* SubEventData) {
  if (Settings->VerboseLevel() > 2)
    cout << endl << "start of " << __PRETTY_FUNCTION__ << endl;
  ...
  EndOfSubEvent = ((unsigned short*)SubEventData) +
                  (SubEventWordCount * sizeof(unsigned short));
  ...
  while (SubEventWordCount > 0) {
    // read first 2 words of buffer header (1.: number of words in buffer, 2.:
    // module number)
    Start = q;
    Length = *q++;
    ...
    if (Settings->VerboseLevel() > 2) {
      cout << "remaining SubEventWordCount: " << SubEventWordCount
           << "; act.Dgf buffer with Length: ";
      cout << Length << " words (raw data ModuleNumber: " << ModuleNumber << ")"
           << endl;
    }

    // check that data in actual buffer doesn't exceed buffer end
    if (NextModule > EndOfSubEvent) {
      cout << __PRETTY_FUNCTION__ << ": XIA wrong buffer length: " << Length
           << ", Start: " << q;
    ...
    if (Settings->VerboseLevel() > 2) {
      cout << __PRETTY_FUNCTION__ << ": XIA module nb: " << ModuleNumber
           << ", length: " << Length;
      cout << ", type: " << SubEventType << ", bufferformat: " << BufferFormat
           << endl;
    }
    ...
    if ((BufferFormat != STD_LM_BUFFORMAT) &&
      ...
      cout << __PRETTY_FUNCTION__ << ": read out event " << EventNumber
           << ": wrong buffer format: ";
      ...
      // read words 3-5 of buffer header: 3.: high-, 4.: middle-, 5.: low-word
      // of run start time
      RunTimeA = *q++;  // high
      RunTimeB = *q++;  // mid
      RunTimeC = *q++;  // low
      ...
      if (fBufferTime == 0) fBufferTime = BufferTime;
    }

    // from here on: always check buffer format (=RUNTASK) before filling 'dgf'
    // or 'dgf_rt259' beam dump module has format 'COMP3_LM_BUFFORMAT' determine
    // corrected module number
    ModuleNumber -= Settings->MarabouDgfModuleOffset();

    if (Settings->VerboseLevel() > 2) {
      cout << __PRETTY_FUNCTION__
           << ": module number for filling 'dgf' or 'dgf_rt259': "
           << ModuleNumber;
    ...
    if (ModuleNumber < (unsigned short)Settings->NumberOfDgfModules()) {
      ...
      cerr << __PRETTY_FUNCTION__ << ": Error, Dgf module number "
           << ModuleNumber;
      cerr << " out of bounds (0 - " << Settings->NumberOfDgfModules()
           << "), skipping event" << endl;
      break;
    }
    ...
    while (q < NextModule) {
      ...
      // read 3 words of event header (1.: hitpattern, 2.: high-, 3.: low-word
      // of event time)
      HitPattern = *q++;
      ...
      if (Settings->VerboseLevel() > 3) {
        cout << __PRETTY_FUNCTION__ << ": XIA hitpattern: 0x" << hex
             << HitPattern << dec << endl;
        cout << __PRETTY_FUNCTION__
             << ": XIA event_time high: " << EventTimeHigh
             << " low: " << EventTimeLow << endl;
      }

      // set 'eventtime'
      EventTime =
          (((unsigned int)EventTimeHigh) << 16) | ((unsigned int)EventTimeLow);
      ...
      if (EventTime > BufferTime)
        ...
        // => if this has an overflow we're screwed anyway so there's no need to
        // check for that
        RunTimeA++;
        CurrentSubEvent.SetEventTime(RunTimeA, EventTimeHigh, EventTimeLow);
      }
      ...
      if (Settings->VerboseLevel() > 4) {
        cout << __PRETTY_FUNCTION__ << ": CurrentSubEvent.GetEventTime() = "
             << CurrentSubEvent.GetEventTime() << endl;
      }

      // check hitpattern: at least one channel bit has to be set for all but
      // TS_EBIS_T1_T2_MODULE and 4 CD TS-modules _all_ timestamp modules with
      // RUNTASK!=259 -> 'ModuleNumber' can be directly compared with 'analysis
      // module number'
      if (!(HitPattern & 0xf) && !Settings->IsTimestampModule(ModuleNumber)) {
        cout << __PRETTY_FUNCTION__
             << ": XIA hitpattern error: hitpattern = " << HitPattern << endl;
        cout << __PRETTY_FUNCTION__ << ": module: " << ModuleNumber
             << ", q: " << q;
        cout << ", NextModule: " << NextModule
             << ", EndOfSubEvent: " << EndOfSubEvent << endl;
        WrongHitPattern++;
        ...
        for (ChannelNumber = 0; ChannelNumber < Settings->NumberOfDgfChannels();
             ChannelNumber++) {
          ...
          if ((HitPattern & ChannelMask) != 0) {
            ...
            if (BufferFormat != COMP3_LM_BUFFORMAT) ChannelLength = *q++;
            ...
            if (Settings->VerboseLevel() > 4) {
              cout << __PRETTY_FUNCTION__ << ": XIA module " << ModuleNumber
                   << " channel " << ChannelNumber;
              cout << " event "
                   << DgfModules[ModuleNumber].GetNumberOfSubEvents()
                   << ": FTtime: ";
              cout << CurrentSubEvent.GetFastTriggerTime(ChannelNumber)
                   << ", energy: ";
              cout << CurrentSubEvent.GetEnergy(ChannelNumber) << endl;
            }

            // get 48bit fast trigger time
            CurrentSubEvent.SetLongFastTriggerTime(ChannelNumber, RunTimeA,
                                                   EventTimeHigh, EventTimeLow);
            ...
            if (Settings->VerboseLevel() > 4) {
              ...
              cout << " event "
                   << DgfModules[ModuleNumber].GetNumberOfSubEvents()
                   << ": lFTtime: ";
              cout << CurrentSubEvent.GetLongFastTriggerTime(ChannelNumber)
                   << endl;
            }
            ...
            if (BufferFormat != COMP3_LM_BUFFORMAT) {
              ...
              if (TraceLength != 0) {
                if (Settings->VerboseLevel() > 4) {
                  cerr << __PRETTY_FUNCTION__ << ": XIA module " << ModuleNumber
                       << " channel ";
                  cerr << ChannelNumber << ": tracelength = " << TraceLength
                       << endl;
      ...
      if (Settings->VerboseLevel() > 4) {
        if (DgfModules[ModuleNumber].GetNumberOfSubEvents() > 0) {
          cout << "added "
               << DgfModules[ModuleNumber].GetNumberOfSubEvents() - 1
               << ". subevent , q = ";
          cout << q << ", NextModule = " << NextModule
               << ", diff = " << NextModule - q << endl;
          cout << *(DgfModules[ModuleNumber].GetSubEvent(
              DgfModules[ModuleNumber].GetNumberOfSubEvents() - 1));
        }
    ...
    if (Length &
        1) {  // dgf buffers are aligned to 32 bit (even number of shorts)

      if (Settings->VerboseLevel() > 2) {
        cout << "Length = " << Length << " => skipping one " << sizeof(q)
             << " byte word" << endl;
      }
  ...
  if (Settings->VerboseLevel() > 1) {
    cout << __PRETTY_FUNCTION__ << ": in read out event " << EventNumber
         << ": read " << Settings->NumberOfDgfModules() << " DGF modules"
         << endl;
  }
...
bool UnpackedEvent::DecodeAdc(int SubEventID, int SubEventWordCount,
                              char* SubEventData) {
  if (Settings->VerboseLevel() > 2) {
    cout << endl << "start of " << __PRETTY_FUNCTION__ << endl;
  }
  ...
  if (Settings->VerboseLevel() > 1) {
    cout << __PRETTY_FUNCTION__ << ": read out event nr. " << EventNumber
         << ": Subevent " << SubEventID
         << ": Caen V7X5 data, word count = " << SubEventWordCount << endl;
    //      for(i = 0; i < 10; i++)
  ...
  while (SubEventWordCount > 0) {
    if (Settings->VerboseLevel() > 2) {
      cout << endl
           << "remaining SubEventWordCount at start of new loop: "
           << SubEventWordCount << endl;
    }
    CurrentSubEvent.ClearEvt();
    // read Header word
    Header = (*q++) << 16;  // Header word (high word)  (data in output buffer:
                            // 32 bit, but q 16 bit pointer)
    ...
    // get number of Channels for which data follows (= WordCount = 32 bit "word
    // count"; bits[13...8] of Header word)
    ...
    // get module number: bits[7...0] of Header word
    // (-caen_module_number_offset)
    ...
    if (0 > ModuleNumber || ModuleNumber >= Settings->NumberOfAdcModules()) {
      cerr << __PRETTY_FUNCTION__ << ": Error, Adc module number "
           << ModuleNumber << " out of bounds (0 - "
           << Settings->NumberOfAdcModules() << "), skipping module" << endl;
      // wc channels plus trailer word (*2 because sevtWc is in 32bit words, q
      // in 16 bit)
    ...
    if (Settings->VerboseLevel() > 2) {
      cout << "Header: 0x" << hex << Header << dec
           << ", crate nr.: " << CrateNumber << ", #of Channels: " << WordCount
           << ", module nr.: " << ModuleNumber << " (before offset correction: "
           << ModuleNumber + Settings->MarabouAdcModuleOffset() << ")" << endl;
    }

    // check type of word (Header: 010) -> skip TOTAL subevent if wrong Header
    if (((Header >> CAEN_V7X5_SH_TYPE_OF_WORD) & CAEN_V7X5_M_TYPE_OF_WORD) !=
        CAEN_V7X5_D_HDR) {
      if (Settings->VerboseLevel() > 0) {
        cout << __PRETTY_FUNCTION__ << ": read event nr. " << EventNumber
             << ": wrong Header word of type: "
             << ((Header >> CAEN_V7X5_SH_TYPE_OF_WORD) &
                 CAEN_V7X5_M_TYPE_OF_WORD)
             << " -> skip TOTAL subevent" << endl;
      }
      WrongAdcHeader++;
      return true;
    }

    // check number of Channels (CAEN7X5_NBOFCHAN=32) -> skip TOTAL subevent if
    // wrong number of Channels
    if ((WordCount < 0 || WordCount > CAEN7X5_NBOFCHAN)) {
      cout << __PRETTY_FUNCTION__ << ": read event nr. " << EventNumber
           << ": wrong WordCount: " << WordCount << " -> skip TOTAL subevent"
           << endl;
      return true;
    }

    // all Header word and eventnumber checks passed successfully -> process
    // event

    // threw out the module number check (which will be performed later) and the
    // event number check (which is not necessary any more)

    // loop over number of Channels for which data follows
    for (ch = 0; ch < WordCount; ch++) {
      // get 32 bit data word
      if (Settings->VerboseLevel() > 3) {
        cout << "unsigned short* q before it gets 'data' word nr. " << ch
             << " of module " << ModuleNumber << ": " << q << endl;
      }

      data = ((unsigned int)*q++)
             << 16;                  // data (high word) (data is 32bit word)
      data |= ((unsigned int)*q++);  // data (low word)

      // check type of word ("valid datum": 000) -> skip actual Channel if wrong
      // type of word
      if (((data >> CAEN_V7X5_SH_TYPE_OF_WORD) & CAEN_V7X5_M_TYPE_OF_WORD) !=
          CAEN_V7X5_D_VALID_DATUM) {
        cout << __PRETTY_FUNCTION__ << ": read event nr. " << EventNumber
             << ": in 'data' word nr. " << ch
             << ": word type not of type 'valid datum' but of type: "
             << ((data >> CAEN_V7X5_SH_TYPE_OF_WORD) & CAEN_V7X5_M_TYPE_OF_WORD)
             << " -> skip actual Channel" << endl;
        continue;
      }
      ...
      // check overflow and underthreshold bit -> skip act.ch. if Channel in
      // overflow or under threshold
      if (data & CAEN_V7X5_B_OVERFLOW) {
        if (Settings->VerboseLevel() > 0) {
          cout << __PRETTY_FUNCTION__ << ": read event nr. " << EventNumber
               << ": CAEN module nr.: " << ModuleNumber
               << " , Channel: " << Channel << ": in 'data' word nr. " << ch
               << ": overflow bit set (ADC value: " << ADCvalue << ")" << endl;
        }
        AdcOverflow++;
      }
      if (data & CAEN_V7X5_B_UNDERTHRESH) {
        if (Settings->VerboseLevel() > 0) {
          cout << __PRETTY_FUNCTION__ << ": read event nr. " << EventNumber
               << ": CAEN module nr.: " << ModuleNumber
               << " , Channel: " << Channel << ": in 'data' word nr. " << ch
               << ": undershoot bit set (ADC value: " << ADCvalue << ")"
               << endl;
        }
        AdcUnderflow++;
      }

      // assign EX_EV variables of struct CMULTI: numofevents,
      // eventnumber[CAENMAXEVENTNUM] and value[CAENMAXEVENTNUM] NOTICE:
      // eventnumber is starting from 1!
      CurrentSubEvent.Add(Channel, ADCvalue);

      if (Settings->VerboseLevel() > 3) {
        cout << "Number of sub events: " << CurrentSubEvent.Size()
             << ", last subevent: channel = "
             << CurrentSubEvent.GetLastAdcChannel()
             << ", value = " << CurrentSubEvent.GetLastAdcValue() << endl;
      }
    }  // for(ch=0; ch<WordCount; ch++)
    ...
    if (((trailer >> CAEN_V7X5_SH_TYPE_OF_WORD) & CAEN_V7X5_M_TYPE_OF_WORD) !=
        CAEN_V7X5_D_EOB) {
      cout << __PRETTY_FUNCTION__ << ": read event nr. " << EventNumber
           << ": wrong EOB word of type: "
           << ((trailer >> CAEN_V7X5_SH_TYPE_OF_WORD) &
               CAEN_V7X5_M_TYPE_OF_WORD)
           << " -> skip TOTAL subevent" << endl;
      return true;
    }
    ...
    ProcessedWordCount =
        (WordCount + 2) *
        2;  // WordCount channels plus 2 (Header + trailer word), *2 because
            // SubEventWordCount is in 16bit words
    SubEventWordCount -=
        ProcessedWordCount;  // remaining subevent data = #of words at start of
                             // while-loop minus #of processed words
...
bool UnpackedEvent::DecodeMesytecAdc(int SubEventID, int SubEventWordCount,
                                     char* SubEventData) {
  if (Settings->VerboseLevel() > 2) {
    cout << endl << "start of " << __PRETTY_FUNCTION__ << endl;
  }
  ...
  if (Settings->VerboseLevel() > 1) {
    cout << __PRETTY_FUNCTION__ << ": read out event nr. " << EventNumber
         << ": Subevent ";
    cout << SubEventID << ": MADC data, word count = " << SubEventWordCount
         << endl;
  }
  ...
  while (SubEventWordCount > 0) {
    ...
    Header = *q++;  // Header word (high word)  (data in output buffer: 32 bit,
                    // but q 16 bit pointer)
    ...
    if (0 > ModuleNumber || ModuleNumber >= Settings->NumberOfAdcModules()) {
      cerr << __PRETTY_FUNCTION__ << ": Error, Adc module number "
           << ModuleNumber;
      cerr << " out of bounds (0 - " << Settings->NumberOfAdcModules()
           << "), skipping module" << endl;
    ...
    if (Settings->VerboseLevel() > 2) {
      ...
      cout << "Header: 0x" << hex << Header << dec
           << ", #of Channels: " << WordCount;
      cout << ", module nr.: " << ModuleNumber
           << " (before offset correction: ";
      cout << ModuleNumber + Settings->MarabouAdcModuleOffset() << ")" << endl;
    }

    // check number of Channels (MESYTEC_MADC_NBOFCHAN=32 + 1 word End of Event
    // + 1 extended timestamp) -> skip TOTAL subevent if wrong number of
    // Channels
    ...
    // loop over number of Channels for which data follows (but WordCount
    // includes the end of event => -1)
    for (ch = 0; ch < WordCount - 1; ch++) {
      ...
      if ((data & MESYTEC_MADC_END_OF_EVENT) == MESYTEC_MADC_END_OF_EVENT) {
        cerr << "Error, found end of event data (" << data << ") after "
             << ch + 1;
        ...
        return false;
               ...
               MESYTEC_MADC_END_OF_EVENT) {  // means only one of the higest two
                                             // bits is non-zero
      ...
      if (data & MESYTEC_MADC_OUT_OF_RANGE) {
        if (Settings->VerboseLevel() > 0) {
          cout << __PRETTY_FUNCTION__ << ": read event nr. " << EventNumber;
          cout << ": CAEN module nr.: " << ModuleNumber
               << " , Channel: " << Channel;
          cout << ": in 'data' word nr. " << ch
               << ": overflow bit set (ADC value: ";
          cout << ADCvalue << ")" << endl;
        }
    ...
    if ((trailer & MESYTEC_MADC_END_OF_EVENT) != MESYTEC_MADC_END_OF_EVENT) {
      cout << __PRETTY_FUNCTION__ << ": read event nr. " << EventNumber;
      cout << ": wrong EOE word of type: "
           << (trailer & MESYTEC_MADC_END_OF_EVENT);
    ...
    // determine number of words that were processed in this while-loop
    ProcessedWordCount =
        (WordCount + 1) * 2;  // WordCount channels plus 1 (Header), *2 because
                              // SubEventWordCount is in 16bit words
    SubEventWordCount -=
        ProcessedWordCount;  // remaining subevent data = #of words at start of
                             // while-loop minus #of processed words
...
bool UnpackedEvent::DecodeSIS3600PatternUnit(int SubEventID,
                                             int SubEventWordCount,
                                             char* SubEventData) {
  if (Settings->VerboseLevel() > 2) {
    cout << endl << "start of " << __PRETTY_FUNCTION__ << endl;
  }
  ...
  if (Settings->VerboseLevel() > 1) {
    cout << "read event nr. " << EventNumber << ": Subevent " << SubEventID
         << ": SIS3600 Pattern Unit data, WordCount = " << SubEventWordCount
         << endl;
  }
  ...
  while (SubEventWordCount > 0) {
    ...
    // Header word: bits 0-7: ModuleSerial, bits 8-14: ModuleID, bit 15: has to
    // be "1" to be OK check Header word
    if ((Header & SIS3600_D_HDR) == 0) {
      cout << __PRETTY_FUNCTION__ << ": read event nr. " << EventNumber
           << ": wrong Header word of type 0x (" << Header
           << ") -> skip TOTAL subevent" << endl;
      return true;
    }
    ...
    if (0 > ModuleNumber || ModuleNumber >= Settings->NumberOfPatternUnits()) {
      cerr << __PRETTY_FUNCTION__ << ": Error, pattern unit number "
           << ModuleNumber << " out of bounds (0 - "
           << Settings->NumberOfPatternUnits() << "), skipping module" << endl;
    ...
    // set number of words that were processed in actual while-loop
    // ("ProcessedWordCount")
      ...
      for (; WordCount > 0; WordCount -= 2) {
        ...
        if (Settings->VerboseLevel() > 3) {
          cout << "number of pattern unit events: " << CurrentSubEvent.Size()
               << ", last value = " << CurrentSubEvent.GetLastValue() << endl;
        }
...
bool UnpackedEvent::DecodeBraggChamber(int SubEventID, int SubEventWordCount,
                                       char* SubEventData) {
  if (Settings->VerboseLevel() > 2) {
    cout << endl << "start of " << __PRETTY_FUNCTION__ << endl;
  }
  ...
  if (Settings->VerboseLevel() > 1) {
    cout << "Subevent " << SubEventID
         << ": bragg chamber data, WordCount = " << SubEventWordCount << endl;
  }

  if (Settings->VerboseLevel() > 0) {
    cout << "================    BRAGG CHAMBER (" << SubEventWordCount
         << ")    ================" << endl;
  }
  ...
  while (SubEventWordCount > 0)  // subevent may contain several SIS modules
  {
    ...
    if (WordCount <= 0) {
      cerr << __PRETTY_FUNCTION__
           << " [SubEventWordCount = " << SubEventWordCount
           << "] Wrong word count: " << WordCount << endl;
    ...
    ModuleNumber =
        header & BRAGG_CHAMBER_MODULE_NUMBER;  // extract module number
    if (ModuleNumber <= 0) {
      cerr << __PRETTY_FUNCTION__
           << " [SubEventWordCount = " << SubEventWordCount
           << "] Wrong module number: " << ModuleNumber << endl;
      return false;
    }
    ...
    {
      cerr << __PRETTY_FUNCTION__ << "[Module " << ModuleNumber
           << ", SubEventWordCount = " << SubEventWordCount
           << "] Wrong acquisition mode - gate chaining not yet implemented"
           << endl;
      return false;
    }
    ...
    StartOfEventBuffer =
        StartOfTriggerBuffer + *StartOfTriggerBuffer;  // start of event data

    for (unsigned short EventNumber = 0; EventNumber < NumberOfEvents;
         EventNumber++) {
      EventEntry =
          *(StartOfTriggerBuffer + EventNumber + 1);  // get event entry
      ...
      if (Settings->VerboseLevel() > 2) {
        cout << "event " << EventNumber << ": StartOfData = " << StartOfData
             << ", EndOfData = " << EndOfData << ", End = " << End << endl;
      }

      while (q < End) {
        // rudis code is: NextEvent = q + (*q++)
        // lets assume the first word is the event length meaning that in above
        // statement the increment is expected to be executed last
        NextEvent = q + (*q);
        q++;
        ChannelGroup = *q++;
        TriggerMode = *q++;
        ...
        if (Settings->VerboseLevel() > 2) {
          cout << "ChannelGroup = " << ChannelGroup
        ...
        for (int i = 0; i < 2; i++) {
          ...
          if (ChannelPointer[i] == NULL) {
            cerr << __PRETTY_FUNCTION__ << ": [Module " << ModuleNumber
                 << ": Channel not defined - ch" << Channel[i]
                 << " - data can't be stored" << endl;
            return false;
          }
          if ((*(ChannelPointer[i])).size() <
              (unsigned int)Settings->BraggTraceLength()) {
        ...
        if (Wrapped) {
          for (p = EndOfData; p < (StartOfData + PageSize); p++, Sample++) {
            ...
            for (int i = 0; i < 2; i++) {
              ...
              CurrentSubEvent.Overflow(
                  i, (((SampleValue >> 16) & OverflowMask[i]) != 0));
            }
          }
        }
    ...
    SubEventWordCount -= WordCount * sizeof(int) /
                         sizeof(short);  // count for WordCount 32 bit words
  }
  return true;
}

// subevent 15, type [10,11]: scaler data: 3 scalers a 32 chn a 32 bit (last two
// are from ppac, not used???)
bool UnpackedEvent::DecodeScaler(int SubEventID, int SubEventWordCount,
                                 char* SubEventData) {
  if (Settings->VerboseLevel() > 2)
    cout << endl << "start of " << __PRETTY_FUNCTION__ << endl;
  ...
  if (SubEventWordCount % 2 != 0) {  // there are two words for each channel =>
                                     // total number of words must be even

    cerr << __PRETTY_FUNCTION__;
    cerr << ": Error, I'm expecting even number of words in SubEvent but there "
            "are ";
...
bool UnpackedEvent::DecodeDgfScaler(int SubEventID, int SubEventWordCount,
                                    char* SubEventData) {
  if (Settings->VerboseLevel() > 2) {
    cout << endl << "start of " << __PRETTY_FUNCTION__ << endl;
  }
  ...
  if (Settings->VerboseLevel() > 1) {
    cout << "Subevent " << SubEventID
         << ": internal dgf counters, WordCount = " << SubEventWordCount
         << endl;
  }
  ...
  if (Settings->VerboseLevel() > 0) {
    cout << "================    DGF SCALER (" << SubEventWordCount
         << ")    ================" << endl;
  }
  ...
  while (*q != DGF_SCALER_END_OF_BUFFER &&
         q - ((unsigned short*)SubEventData) <
             SubEventWordCount)  // end of data reached?
  {
    if (Settings->VerboseLevel() > 2) {
      cout << "next loop iteration " << q - ((unsigned short*)SubEventData)
           << endl;
    }

    if (*q++ != DGF_SCALER_MAGIC_WORD)  // check if we are still in phase
    {
      cerr << "Internal dgf scalers: data out of phase - 0x" << hex << *--q
           << " (should be magic word 0x" << DGF_SCALER_MAGIC_WORD << ")" << dec
           << endl;
      return false;
    }
    ...
    if (DgfID <= 0 || DgfID > Settings->NumberOfDgfModules()) {
      cerr << "Dgf index out of phase - " << DgfID << " (should be in [1,"
           << Settings->NumberOfDgfModules() << "])" << endl;
      return false;
    }

    if (Settings->VerboseLevel() > 2) {
      cout << "Cluster " << ClusterID << ", Dgf " << DgfID
    ...
    if (Settings->VerboseLevel() > 2) {
      cout << "RealTime: reading q[" << Settings->RealTimeIndex() << " - "
           << Settings->RealTimeIndex() + 2 << "]" << endl;
    }
    CurrentSubEvent.RealTime(
        q[Settings->RealTimeIndex()] |
        ((unsigned long long)q[Settings->RealTimeIndex() + 1]) << 16 |
        ((unsigned long long)q[Settings->RealTimeIndex() + 2]) << 32);
    if (Settings->VerboseLevel() > 2) {
      cout << "RunTime: reading q[" << Settings->RunTimeIndex() << " - "
           << Settings->RunTimeIndex() + 2 << "]" << endl;
    }
    CurrentSubEvent.RunTime(
        q[Settings->RunTimeIndex()] |
        ((unsigned long long)q[Settings->RunTimeIndex() + 1]) << 16 |
        ((unsigned long long)q[Settings->RunTimeIndex() + 2]) << 32);
    if (Settings->VerboseLevel() > 2) {
      cout << "GSLTTime: reading q[" << Settings->GSLTTimeIndex() << " - "
           << Settings->GSLTTimeIndex() + 2 << "]" << endl;
    }
    CurrentSubEvent.GSLTTime(
        q[Settings->GSLTTimeIndex()] |
        ((unsigned long long)q[Settings->GSLTTimeIndex() + 1]) << 16 |
        ((unsigned long long)q[Settings->GSLTTimeIndex() + 2]) << 32);
    if (Settings->VerboseLevel() > 2) {
      cout << "NumberOfEvents: reading q[" << Settings->NumberOfEventsIndex()
           << " - " << Settings->NumberOfEventsIndex() + 1 << "]" << endl;
    }
    CurrentSubEvent.NumberOfEvents(q[Settings->NumberOfEventsIndex()] |
                                   (int)q[Settings->NumberOfEventsIndex() + 1]);

    for (int i = 0; i < Settings->NumberOfDgfChannels(); i++) {
      if (Settings->VerboseLevel() > 2) {
        cout << "LiveTime " << i << ": reading q["
             << Settings->LiveTimeIndex() + Settings->ChannelIndicesOffset(i)
             << " - "
             << Settings->LiveTimeIndex() + 2 +
                    Settings->ChannelIndicesOffset(i)
             << "]" << endl;
      }
      CurrentSubEvent.LiveTime(
          i, q[Settings->LiveTimeIndex() + Settings->ChannelIndicesOffset(i)] |
                 ((unsigned long long)q[Settings->LiveTimeIndex() + 1 +
                                        Settings->ChannelIndicesOffset(i)])
                     << 16 |
                 ((unsigned long long)q[Settings->LiveTimeIndex() + 2 +
                                        Settings->ChannelIndicesOffset(i)])
                     << 32);
      if (Settings->VerboseLevel() > 2) {
        cout << "FastPeak " << i << ": reading q["
             << Settings->FastPeakIndex() + Settings->ChannelIndicesOffset(i)
             << " - "
             << Settings->FastPeakIndex() + 1 +
                    Settings->ChannelIndicesOffset(i)
             << "]" << endl;
      }
      CurrentSubEvent.FastPeak(
          i, q[Settings->FastPeakIndex() + Settings->ChannelIndicesOffset(i)] |
                 ((unsigned int)q[Settings->FastPeakIndex() + 1 +
                                  Settings->ChannelIndicesOffset(i)])
                     << 16);
    }
    ...
    if (Settings->VerboseLevel() > 2) {
      ...
      cout << "check whether " << flush << *q
           << " != " << DGF_SCALER_END_OF_BUFFER << " and "
           << q - ((unsigned short*)SubEventData) << " < " << SubEventWordCount
           << endl;
    }
  }
...
unsigned short UnpackedEvent::GetTimestamps(
    vector<unsigned short>& Timestamps) {
  unsigned short ts;
  unsigned short SumOfTimestamps = 0;

  if (Timestamps.size() != Settings->NumberOfTimestampModules()) {
    cerr << "Warning, have to resize timestamp vector from "
         << Timestamps.size() << " to " << Settings->NumberOfTimestampModules()
         << endl;
  ...
  for (ts = 0; ts < Settings->NumberOfTimestampModules(); ts++) {
    // for actual ts module: set number of part. timestamps 'nofts[ts]' to nr.
    // of found VME gate hitpatterns, set 'sumofts' and reset
    // 'VMEGateHitpatternCounter'
    Timestamps.at(ts) = GetTimestamps(ts);
    SumOfTimestamps += Timestamps.at(ts);
    if (Settings->VerboseLevel() > 3) {
      cout << ts << ". # of timestamps = " << Timestamps.at(ts)
           << ", sum = " << SumOfTimestamps << endl;
    }
...
unsigned short UnpackedEvent::GetTimestamps(unsigned short ts) {
  ...
  for (unsigned int tse = 0;
       tse < DgfModules[Settings->TimestampModule(ts)].GetNumberOfSubEvents();
       tse++) {
    // if VME gate hitpattern (i.e. timestamps) -> count them if energy > 0
    if (DgfModules[Settings->TimestampModule(ts)]
            .GetSubEvent(tse)
            ->GetHitPattern() &
        (1 << Settings->TimestampChannel()))
      Counter++;
  }
...
// checks the timestamps and the number of adcs (also checks that number of adcs
// comes in multiples of adcsPerTimestampModule)
bool UnpackedEvent::Verify() {
  ...
  // loop over the adcs but only those with timestamper (not the
  // diamond/collimator adc afterwards)
  for (adc = 0; adc < Settings->NumberOfTimestampModules() *
                          Settings->NumberOfAdcsPerTimestampModule();
       adc++) {
    // check number of timestamps and count the number of adc modules with sub
    // events
    ...
    if (AdcModules[adc].GetNumberOfSubEvents() != ts) {
      ...
      cerr << adc / Settings->NumberOfAdcsPerTimestampModule() << " (= " << ts
           << ")" << endl;
      failed = true;
    }
  ...
  // there should always be n*NumberOfAdcsPerTimestampModule(+1) modules in one
  // event!
  if (NumberOfAdcs != 0 &&
      NumberOfAdcs % Settings->NumberOfAdcsPerTimestampModule() != 0 &&
      NumberOfAdcs % Settings->NumberOfAdcsPerTimestampModule() != 1) {
    cerr << __PRETTY_FUNCTION__ << ": in event " << EventNumber - 1;
    cerr << " were the wrong number of ADC modules (" << NumberOfAdcs << ")!"
         << endl;
    failed = true;
  }
...
void UnpackedEvent::CorrectMesytecAdcTimestamps() {
  // loop over all subevents in all modules add the buffer time of the first dgf
  // buffer to the time stamp
  long long timestamp;

  for (unsigned int module = 0; module < AdcModules.size(); module++) {
    for (unsigned int subevent = 0;
         subevent < AdcModules[module].GetNumberOfSubEvents(); subevent++) {
...
void UnpackedEvent::Statistics() {
  ...
  cout << "wrong dgf hit pattern:           " << setw(12) << WrongHitPattern
       << " (" << setw(4);
  cout << setiosflags(ios::fixed) << setprecision(1)
       << 100. * WrongHitPattern / EventNumber << " %)" << endl;
  cout << "wrong adc headers:               " << setw(12) << WrongAdcHeader
       << " (" << setw(4);
  cout << setiosflags(ios::fixed) << setprecision(1)
       << 100. * WrongAdcHeader / EventNumber << " %)" << endl;
  cout << "# of overflows in adc channels:  " << setw(12) << AdcOverflow << " ("
       << setw(4);
  cout << setiosflags(ios::fixed) << setprecision(1)
       << 100. * AdcOverflow / EventNumber << " %)" << endl;
  cout << "# of underflows in adc channels: " << setw(12) << AdcUnderflow
       << " (" << setw(4);
  cout << setiosflags(ios::fixed) << setprecision(1)
       << 100. * AdcUnderflow / EventNumber << " %)" << endl;
