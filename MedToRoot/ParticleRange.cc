...
ParticleRange::ParticleRange() : Emin(200), Estep(10) {}
...
void ParticleRange::Read(const std::string& filename) {
  ...
  std::cout << "ParticleRange: " << values.size() << " interpolation points"
            << std::endl;
}
...
float ParticleRange::GetRange(int energy) const {
  ...
  if (index >= values.size()) return 1e6;
  return values[index];
}
...
int main(int argc, char* argv[]) {
  ...
  const int E[] = {1000,  2000,  3000,  3200,  10000, 30000,
                   40000, 75000, 79000, 81000, 85000, -1};
  for (int i = 0; E[i] > 0; ++i)
