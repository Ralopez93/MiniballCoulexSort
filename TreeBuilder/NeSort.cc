  ...
  for (i = 0; i < 8; i++) {
    for (j = 0; j < 3; j++) {
      for (k = 0; k < 7; k++) {
        ...
        E_gam_seg_core[i][j][k - 1] =
            new TH1F(Form("E_gam_%d_%d_%d_core", i, j, k),
                     Form("E_gam_%d_%d_%d_core", i, j, k), GBINS, GMIN, GMAX);
      ...
      if (0 <= dgf_num && dgf_num < 48 && 0 <= dgf_ch && dgf_ch < 4) {
        ...
        if (dgf_num % 2 == 0 && dgf_ch > 0 && dgf_ch < 3) {  // seg1 and seg2
          ...
          if (dgf_ch == 0)
            ...
            E_gam_seg_core[dgf_num / 6][dgf_num % 6 / 2][dgf_ch - 1]->Fill(
                GammaEnergyCore[dgf_num / 2]);
          ...
          E_gam_seg_cal[dgf_num / 6][dgf_num % 6 / 2][dgf_ch + 3]->Fill(
              GammaEnergy);
          E_gam_seg_core[dgf_num / 6][dgf_num % 6 / 2][dgf_ch + 3 - 1]->Fill(
              GammaEnergyCore[dgf_num / 2]);
