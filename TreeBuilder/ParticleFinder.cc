...
void ParticleFinder::FindTREXParticles() {
  ...
  for (unsigned int i = 0; i < subevent->Size(); i++) {
    ...
    // cout << "ADC #" << adc_num << "; adc_ch = " << adc_ch << "; adc_en = " <<
    // adc_en << endl;
...
unsigned int ParticleFinder::ReconstructCD(bool trex, bool crex) {
  ...
  // Forwards CD //
  /////////////////
  //	cout<<fcdfrontenergy.size()<<" "<<fcdbackenergy.size()<<endl;
  ...
  if (fcdfrontenergy.size() == 1 && fcdbackenergy.size() == 1) {
    ...
    E_f_b[adc_num]->Fill(fcdfrontenergy.at(0) / 1000.,
                         fcdbackenergy.at(0) / 1000.);
  ...
  // one quadrant only has a single dead strip, for example
           ...
           (padenergy[0] > 0 || padenergy[1] > 0 || padenergy[2] > 0 ||
            padenergy[3] > 0) &&
           PEn.size() == 0)
    cd_debug->Fill(4);
  ...
  if (bcdfrontenergy.size() == 1 && bcdbackenergy.size() == 1) {
    ...
    // E_f_b[adc_num]->Fill( bcdfrontenergy.at(0)/1000.,
    // bcdbackenergy.at(0)/1000. );
  ...
  // 1 on the front and 2 on the back
    ...
    // E_f_b[adc_num]->Fill( bcdfrontenergy.at(0)/1000.,
    // bcdbackenergy.at(0)/1000. ); E_f_b[adc_num]->Fill(
    // bcdfrontenergy.at(0)/1000., bcdbackenergy.at(1)/1000. );
  ...
  // 2 on the front and 1 on the back
    ...
    // E_f_b[adc_num]->Fill( bcdfrontenergy.at(0)/1000.,
    // bcdbackenergy.at(0)/1000. ); E_f_b[adc_num]->Fill(
    // bcdfrontenergy.at(1)/1000., bcdbackenergy.at(0)/1000. );
