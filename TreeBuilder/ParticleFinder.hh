...
void ParticleFinder::InitialiseHistograms() {
  ...
  part =
      new TH2F("part", "particle singles", 64, -0.5, 63.5, PBINS, PMIN, PMAX);
  part_raw = new TH2F("part_raw", "particle singles before reconstruction", 64,
                      -0.5, 63.5, PBINS, PMIN, PMAX);
  ...
  for (unsigned int i = 0; i < 10; i++) {
    for (unsigned int j = 0; j < 32; j++) {
      adc_spec[i][j] = new TH1F(Form("adc_%d_%d", i, j),
                                Form("adc_%d_%d", i, j), 4096, -0.5, 4095.5);
      adc_spec[i][j]->GetXaxis()->SetTitle("Energy Particles [Channels]");
      adc_spec_cal[i][j] =
          new TH1F(Form("adc_%d_%d_cal", i, j), Form("adc_%d_%d_cal", i, j),
                   PBINS, PMIN, PMAX);
  ...
  for (unsigned int i = 0; i < 4; i++) {
    CD_front_energy[i] =
        new TH2F(Form("CD_front_energy_%d", i), Form("CD_front_energy_%d", i),
                 32, -0.5, 31.5, PBINS, PMIN, PMAX);
    CD_front_energy[i]->GetXaxis()->SetTitle("Ring number");
    CD_front_energy[i]->GetYaxis()->SetTitle("Energy particle [MeV]");
    CD_back_energy[i] =
        new TH2F(Form("CD_back_energy_%d", i), Form("CD_back_energy _%d", i),
                 32, -0.5, 31.5, PBINS, PMIN, PMAX);
    CD_back_energy[i]->GetXaxis()->SetTitle("Strip number");
    CD_back_energy[i]->GetYaxis()->SetTitle("Energy particle [MeV]");

    E_f_b[i] = new TH2F(Form("E_f_b_%d", i), Form("E_f_b_%d", i), PBINS, PMIN,
                        PMAX, PBINS, PMIN, PMAX);
  ...
  for (unsigned int i = 0; i < 4; i++) {
    Barrel_energy[i] =
        new TH2F(Form("Barrel_energy_%d", i), Form("Barrel_energy_%d", i), 32,
                 -0.5, 31.5, PBINS, PMIN, PMAX);
  ...
  ic_dir->cd();
  dEE = new TH2F("dEE", "ionisation chamber;E_{rest};delta-E", 4096, -0.5,
                 4095.5, 4096, -0.5, 4095.5);
