...
void AddBack::MakeGammaRays( bool addback, bool reject, bool segsum ) {
  ...
  for( unsigned int j = 0; j < event->NumberOfDgfs(); j++ ) {
	...
	if (!(dgf_num/2==15 && dgf_ch==0) && !(dgf_num/2==17 && dgf_ch==0)){ // pause filling crystal 15 core gammas
	...
	for( unsigned int k = 0; k < event->NumberOfDgfs(); k++ ) {
	  ...
	  else if ( dgf_num2/2==17){ // crystal 17, needs crosstalk correction
	    if (dgf_num2%2==1){
	      if (dgf_ch2==1){
		c17_s4 = dgf_en2;
		c17_s45 += dgf_en2;
	...
	if (c15SegSumDgf_s12 > 0 || c15SegSumDgf_s3456 > 0){
	  double fc15 = C15XTALK*((c15SegSumDgf_s12-c15SegSumDgf_s3456)/(c15SegSumDgf_s12+c15SegSumDgf_s3456));
	  //	  cout<<c15SegSumDgf_s12<<" "<<c15SegSumDgf_s3456<<" "<<fc15<<endl;
	  //	  cout<<"previous c15 dgf_en: "<<dgf_en<<endl;
	...
	if (MaxSegClu==5 && MaxSegCore==2 && SegSumEnergy==0) continue; // skip core-only events
	...
	if (c17_s4 > 0 || c17_s5 > 0){
	  if (c17_s4 > 0 && c17_s5==0){
	    ...
	    if (frac > C17S4CUT)
	      ...
	      dgf_en =  dgf_en*(1+C17S4XTALK*((1.-C17S4CUT)+(frac-C17S4CUT)/C17S4CUT*(1.-C17S4CUT)));
	    ...
	    if (frac > C17S5CUT)
	      ...
	      dgf_en =  dgf_en*(1+C17S5XTALK*((1.-C17S5CUT)+(frac-C17S5CUT)/C17S5CUT*(1.-C17S5CUT)));  
	  }
	  else{
	...
	if (dead_segments[MaxSegClu][MaxSegCore].size()==1){ // one dead segment, check if missing energy is greater
	  if (MaxSegEnergy < GammaEnergy/(double)(seg_mul+1.)){ // more energy in missing segment suspected; check 40%(dead)/30%/30% distributions
	...
	else if (dead_segments[MaxSegClu][MaxSegCore].size()>=2){ // ambiguity in dead segments, check energy
	  if (MaxSegEnergy < GammaEnergy/(double)(seg_mul+1.)){ // more energy in missing segments suspected
	...
	//	else if( segsum && !reject ) continue; // assuming energy imbalance due to dead segment and not random signal
				
	// If using reject and segsum in combination, take only single-segment hits
	if( segsum && reject ) {
	...
	for( unsigned int l = 0; l < gen_array.size(); l++ ) {
	  ...
	  if( clu_array[l] == MaxSegClu && addback && !ab_array[l] ) {
	    ...
	    if( sen_array[l] < MaxSegEnergy ) { // update with higher-energy segment info
	...
	if( ab_evt ) continue; // get next gamma (this one (j) has been added back to the previous gamma with same cluster)
  ...
  for( unsigned int i = 0; i < gen_array.size(); i++ ) {
    ...
    E_gam_vs_seg->Fill( cid_array[i] * 7 + sid_array[i], gen_array[i] ); // segment
