...
class ParticleGammaTree : public TObject {
 public:
  ...
  /// Setup sorting flags
  virtual void SetupFlags(bool _singles, bool _gamgam, bool _addback,
                          bool _reject, bool _segsum, bool _crex, bool _trex,
  ...
  double tMinPromptElectron;  ///< minimum edge of prompt window
                              ///< (particle-electron)
  double tMaxPromptElectron;  ///< maximum edge of prompt window
                              ///< (particle-electron)
  double tMinRandomElectron;  ///< minimum edge of random window
                              ///< (particle-electron)
  double tMaxRandomElectron;  ///< maximum edge of random window
                              ///< (particle-electron)
...
void ParticleGammaTree::InitialiseVariables() {
  ...
  tMinPrompt = -12.;
  tMaxPrompt = 6.;  // 18 ticks
  tMinRandom = 10.;
  tMaxRandom = 34.;  // 24 ticks
  tMinDelayed = -31.;
  tMaxDelayed = -13.;  // 18 ticks

  tMinPromptElectron = -6.;
  tMaxPromptElectron = 6.;  // 12 ticks
  tMinRandomElectron = 7.;
  tMaxRandomElectron = 33.;  // = (39/18)*12 = 26

  tMinPromptGamma = -15.;
  tMaxPromptGamma = 15.;  // from tdiff_gg
  tMinRandomGamma = 20.;
  tMaxRandomGamma = 35.;  // from tdiff_gg
...
void ParticleGammaTree::SetupFlags(bool _singles, bool _gamgam, bool _addback,
                                   bool _reject, bool _segsum, bool _crex,
                                   bool _trex, bool _cdpad, bool _ionch,
...
void ParticleGammaTree::SetupHistograms() {
  ...
  tdiff_ep->GetXaxis()->SetTitle(
      "time diff (particle - electron) [25 ns tics]");
  tdiff_gg = new TH1F("tdiff_gg", "tdiff_gg", 201, -100.5, 100.5);
  tdiff_gg->GetXaxis()->SetTitle("time diff between gamma rays [25 ns tics]");
  tdiff_pgg = new TH1F("tdiff_pgg", "tdiff_pgg, coincident with particle", 201,
                       -100.5, 100.5);
  tdiff_pgg->GetXaxis()->SetTitle("time diff between gamma rays [25 ns tics]");
  for (unsigned int i = 0; i < 4; i++)
    tdiff_gp_q[i] = new TH1F(Form("tdiff_gp_%d", i), Form("tdiff_gp_%d", i),
                             201, -100.5, 100.5);

  // Gamma-gamma - no Doppler correction
  gg = new TH2F("gg", "#gamma-#gamma matrix;Energy [keV];Energy[keV]", GBINS,
                GMIN, GMAX, GBINS, GMIN, GMAX);
  gg_pr =
      new TH2F("gg_pr", "prompt #gamma-#gamma matrix;Energy [keV];Energy[keV]",
               GBINS, GMIN, GMAX, GBINS, GMIN, GMAX);
  gg_ran =
      new TH2F("gg_ran", "random #gamma-#gamma matrix;Energy [keV];Energy[keV]",
               GBINS, GMIN, GMAX, GBINS, GMIN, GMAX);
