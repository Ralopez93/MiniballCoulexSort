...
class MBGeometry {
 public:
  ...
  void SetupCluster(double user_theta, double user_phi, double user_alpha,
                    double user_r, double user_z);
  ...
  /// Set the distance between the origin and the target position along the beam
  /// axis \param user_z distance from target to origin in beam direction [mm]
  ...
  /// \param core number of the MB Ge crystal counting from 0 to 23
  /// \param seg number of the segment within the crystal: 0 is core, 1-6 for
  /// segments \return theta of segment in beam reference (degrees)
  double GetSegTheta(int core, int seg);

  /// Get the phi angle of a segment with respect to the beam
  /// \param seg number of the segment within the crystal: 0 is core, 1-6 for
  /// segments \return phi of segment in beam reference (degrees)
 ...
 private:
  ...
  double
      z;  ///< distance from target to origin (beam direction is positive) in mm
  ...
  TVector3 mbzoffset;  ///< Offset of target from origin in direction of beam in
                       ///< mm. This shift is independent of the CD detector
                       ///< distance which is relative.
