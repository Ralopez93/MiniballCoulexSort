...
ClassImp(BuiltEvent) ClassImp(AdcData) ClassImp(DgfData)
  ...
  for (i = 0; i < 4; i++) {
    mE_part_ann_cal[i] =
        new TH2F(Form("fE_Q%d", i + 1), Form("Calib. matrix fE_Q%d", i + 1), 16,
                 0, 16, 4000, -0.5, 3999999.5);
    mE_part_ann_cal[i]->GetYaxis()->SetTitle("Energy Particles [keV]");
    mE_part_sec_cal[i] =
        new TH2F(Form("bE_Q%d", i + 1), Form("Calib. matrix bE_Q%d", i + 1), 12,
                 0, 12, 4000, -0.5, 3999999.5);
    mE_part_sec_cal[i]->GetYaxis()->SetTitle("Energy Particles [keV]");
    mE_part_sec_px_cal[i] = new TH2F(
        Form("bE_Q%d_f15", i + 1), Form("Calib. matrix bE_Q%d_f15", i + 1), 12,
        0, 12, 4000, -0.5, 3999999.5);  // strip #15
    mE_part_sec_px_cal[i]->GetYaxis()->SetTitle("Energy Particles [keV]");

    for (j = 0; j < 16; j++) {
      E_part_ann[i][j] =
          new TH1F(Form("fE_Q%d_f%d", i + 1, j + 1),
                   Form("fE_Q%d_f%d", i + 1, j + 1), 4096, -0.5, 4095.5);
      E_part_ann[i][j]->GetXaxis()->SetTitle("Energy Particles [Channels]");

      E_part_ann_cal[i][j] =
          new TH1F(Form("fE_Q%d_f%d_cal", i + 1, j + 1),
                   Form("fE_Q%d_f%d_cal", i + 1, j + 1), 4000, -0.5, 3999999.5);
      E_part_ann_cal[i][j]->GetXaxis()->SetTitle("Energy Particles [keV]");

      for (k = 0; k < 12; k++) {
        E_part_ann_px[i][j][k] = new TH1F(
            Form("fE_Q%d_f%d_b%d", i + 1, j + 1, k + 1),
            Form("fE_Q%d_f%d_b%d", i + 1, j + 1, k + 1), 4096, -0.5, 4095.5);
        E_part_ann_px[i][j][k]->GetXaxis()->SetTitle(
            "Energy Particles [Channels]");

        E_part_ann_px_cal[i][j][k] =
            new TH1F(Form("fE_Q%d_f%d_b%d_cal", i + 1, j + 1, k + 1),
                     Form("fE_Q%d_f%d_b%d_cal", i + 1, j + 1, k + 1), 4000,
                     -0.5, 3999999.5);
        E_part_ann_px_cal[i][j][k]->GetXaxis()->SetTitle(
            "Energy Particles [keV]");
      }
    }

    for (j = 0; j < 12; j++) {
      E_part_sec[i][j] =
          new TH1F(Form("bE_Q%d_b%d", i + 1, j + 1),
                   Form("bE_Q%d_b%d", i + 1, j + 1), 4096, -0.5, 4095.);
      E_part_sec[i][j]->GetXaxis()->SetTitle("Energy Particles [Channels]");

      E_part_sec_cal[i][j] =
          new TH1F(Form("bE_Q%d_b%d_cal", i + 1, j + 1),
                   Form("bE_Q%d_b%d_cal", i + 1, j + 1), 4000, -0.5, 3999999.5);
      E_part_sec_cal[i][j]->GetXaxis()->SetTitle("Energy Particles [keV]");

      for (k = 0; k < 16; k++) {
        E_part_sec_px[i][k][j] = new TH1F(
            Form("bE_Q%d_f%d_b%d", i + 1, k + 1, j + 1),
            Form("bE_Q%d_f%d_b%d", i + 1, k + 1, j + 1), 4096, -0.5, 4095.5);
        E_part_sec_px[i][k][j]->GetXaxis()->SetTitle(
            "Energy Particles [Channels]");

        E_part_sec_px_cal[i][k][j] =
            new TH1F(Form("bE_Q%d_f%d_b%d_cal", i + 1, k + 1, j + 1),
                     Form("bE_Q%d_f%d_b%d_cal", i + 1, k + 1, j + 1), 4000,
                     -0.5, 3999999.5);
        E_part_sec_px_cal[i][k][j]->GetXaxis()->SetTitle(
            "Energy Particles [keV]");
      }
    }
  }
  ...
  for (i = 0; i < nentries; i++) {
    ...
    if (status == -1) {
      ...
      cerr << " in file " << tr->GetFile()->GetName() << " doesn't exist"
           << endl;
      return 6;
    }
        ...
        if (adc_ch < 16) {  // front rings
          ...
          for (l = 0; l < event->Adc(j)->SubEvent()->Size(); l++) {
            ...
            if (adc_ch2 < 16 && adc_en2 > 100) {
              ...
              if (adc_ch2 == 14)
                mE_part_sec_px_cal[adc_num]->Fill(adc_ch - 16,
                                                  energy);  // strip #15
            }
